...
20. КУЗНЕЧИК (ГОСТ Р 34.12-2015)  (ЛАБКРИПТ_2022)

Блочный шифр на основе SP-сети (Substitution-Permutation). Блок 128 бит.
Раунд состоит из трёх преобразований:
    X — наложение раундового ключа сложением по модулю 2 (XOR),
    S — нелинейное преобразование (замена байтов по таблице PI),
    L — линейное преобразование (перемешивание на основе РСЛОС в GF(2^8)).
9 полных раундов (X→S→L) и финальное наложение ключа X. Из 256-битного ключа
развёртываются 10 раундовых ключей по схеме Фейстеля.
"""

import struct

# Таблица нелинейного преобразования (S-блок, преобразование S)
PI = [
