...
10. Вертикальная перестановка  (ЛАБКРИПТ_2022)

Текст построчно вписывается в прямоугольную матрицу (ширина = длина ключа),
затем столбцы считываются в порядке, заданном ключом. Ключевая
последовательность — расположение букв ключа-слова в алфавите.
Последняя строка может быть неполной: при расшифровании учитываются длинные
и короткие столбцы. Число ключей не более m! (m — число столбцов).
"""
...
def get_key_order(key):
    """Преобразует ключ в ключевую последовательность (длина ключа = ширина)"""
