...
22. Elgamal  (ЛАБКРИПТ_2022)

Параметры: простое p (> M_i), 1 < g < p, секретный ключ 1 < x < p.
Открытый ключ:  y = g^x mod p   (открытые: p, g, y; секрет: x).
Для каждой шифрвеличины M_i выбирается случайный рандомизатор k_i, взаимно
простой с φ(p) = p-1:
    a_i = g^k_i mod p,   b_i = y^k_i · M_i mod p.
Шифробозначение — пара (a_i, b_i); шифртекст вдвое длиннее открытого текста.
Расшифрование:  M_i ≡ b_i · (a_i^x)^(-1) (mod p).
"""
