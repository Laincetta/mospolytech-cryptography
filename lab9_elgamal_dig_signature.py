...
25. EGSA — цифровая подпись Эль-Гамаля  (ЛАБКРИПТ_2022)

Хеш — квадратичная свёртка:  h = ((h + m_i)^2) mod (p−1)  по буквам.
Подпись (a, b):  a = g^k mod p,  b = (m − x·a)·k^(−1) mod (p−1),
где k взаимно просто с (p−1). Проверка: (y^a · a^b) mod p == g^m mod p.
"""
import math
import random
