...
27. ГОСТ Р 34.10-2012  (ЛАБКРИПТ_2022)

Цифровая подпись на эллиптической кривой y² = x³ + a·x + b над полем GF(p).
Параметры: точка G порядка q, секретный ключ x, открытый ключ Y = [x]·G.
Подпись сообщения m:
    H(m) — хеш, P = [k]·G, r = x_P mod q, s = (k·H(m) + r·x) mod q.
Проверка: h⁻¹ = H(m)^(q-2) mod q, z1 = s·h⁻¹, z2 = -r·h⁻¹,
    P = [z1]·G + [z2]·Y, подпись верна при x_P mod q = r.
"""
...
def add_points(P, Q, a, p):
    ...
    Сложение:  λ = (y2 - y1)/(x2 - x1),  x3 = λ² - x1 - x2,  y3 = λ(x1 - x3) - y1.
    Удвоение (P = Q):  λ = (3·x1² + a)/(2·y1),  x3 = λ² - 2·x1,  y3 = λ(x1 - x3) - y1."""
    if P is None:
        return Q
    if Q is None:
        return P
...
def multiply_point(k, P, a, p):
    """Умножение точки на скаляр k (композиция точки [k]·P) — аналог
    возведения в степень: точка складывается сама с собой k раз."""
    if P is None or k == 0:
        return None
