...
24. RSA — цифровая подпись  (ЛАБКРИПТ_2022)

Подпись хеша сообщения секретным ключом RSA. Хеш — квадратичная свёртка:
    h = ((h + m_i)^2) mod N   по буквам (m_i — номер буквы в алфавите).
Подпись:    S = h^D mod N.
Проверка:   m = S^E mod N;  подпись верна, если m совпадает с заново вычисленным h.
"""
