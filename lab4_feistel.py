...
12. Перестановка в комбинационных шифрах (DES, МАГМА)  (ЛАБКРИПТ_2022)

В основе — сеть Фейстеля (Х. Фейстель, 1971). Открытый текст делится на левую
и правую половины (здесь a1 ≙ L, a0 ≙ R). На i-м цикле с подключом K_i:
    R_{i+1} = L_i,   L_{i+1} = R_i ⊕ f(R_i, K_i).
Функция f (здесь g[k]): R_{i-1} складывается с K_i по модулю 2^32, результат
разбивается на восемь тетрад → S-блоки → циклический сдвиг на 11 бит влево.
256-битный ключ разбивается на восемь 32-битных подключей K_1…K_8;
при расшифровании порядок подключей инвертируется.
"""
...
def g(k, a):
    """Функция шифрования f (в нотации Магмы — g[k]):
    g[k](a) = rot11(t((a + k) mod 2³²))  — сложение с подключом по модулю 2³²,
    замена по S-блокам, циклический сдвиг на 11 бит влево."""
    sum_mod = (a + k) & MASK32
